  ...
  label-width="120px">
    ...
    <el-form-item
      ...
      <el-input
        ...
        :minlength="2"
        :maxlength="10"
        v-model.number="estInfo.est_amt_without_tax">
        </el-input>
              (税込: {{est_amt_with_tax | formatAmt}})
    </el-form-item>
    </el-col>
    <el-col :span="14">
    <el-form-item
...
<script>
import { EstEntMenu } from '@/webedi/est/components';
import { EstDtl } from '@/webedi/est/components';
  export default {
      components: {
        'est-ent-menu' : EstEntMenu,
        'est-dtl' : EstDtl
      },
  data(){
    //check
  ...
  methods:{
    ent(){
      ...
      _this.estInfo.est_amt_with_tax = _this.est_amt_with_tax;
      _this.$refs.estInfo.validate(
        (valid) => {
          if(valid){
            _this.$confirm('見積情報を登録してもよろしいでしょうか？','注意',{
              ...
              //Post start>>>
              () => {
                _this.$http.post('/api/est/insert', _this.estInfo).then(
                  function(response){
    ...
    loadEstEntInfo() {
        var p_appr_doc_id_list = this.$route.query.appr_doc_id_list;
        if(this.$_.isUndefined(p_appr_doc_id_list)) return;
        var _this = this;
        _this.$http.get('/api/est/selectForInsert', {
            params: {
              appr_doc_id_list: p_appr_doc_id_list
            }
          })
          .then(function(response) {
            ...
            if (errorcode == "200") {
              ...
              _this.setInitValue(_this.estInfo);
      ...
      setInitValue(est_info){
        est_info.est_no = '';
        // VUE.set(est_info,'submit_dt',new Date());
        this.$set(est_info,'submit_dt',new Date());
        this.$set(est_info,'est_k','2');
        this.$set(est_info,'shipping_order_spec_no','');
        this.$set(est_info,'order_content','');
        this.$set(est_info,'special_affairs','');
        this.$set(est_info,'remark','');
        this.$set(est_info,'est_amt_with_tax',0);
      },
  ...
  computed: {
      est_amt_with_tax: function(){
        let p_est_amt_with_tax =  this.estInfo.est_amt_without_tax*(1+this.tax_rate/100);
        return p_est_amt_with_tax;
      }
