    ...
    <el-form
     ...
     <el-table :data="page_info.records"
      ...
      {{(scope.$index +  1)+ (page_info.current - 1) *page_info.size}}
        ...
        <template slot-scope="scope">
        <el-button type="text"
        @click="issueEstDoc(scope.$index,scope.row)" size="mini">発行</el-button>
        </template>
        </el-table-column>
        <el-table-column
        label="送付"
        ...
        <template slot-scope="scope">
        <el-button type="text"
        @click="deliveryEstDoc(scope.$index,scope.row)" size="mini">送付</el-button>
        </template>
        </el-table-column>
        <el-table-column
        label="見積書発行日見積書送付日"
        ...
        @click="delEstData(scope.$index,scope.row)" size="mini" :disabled="scope.row.del_flg === 'Y'" icon="delete">削除</el-button>
        </template>
        </el-table-column>
    </el-table>
  <el-pagination
...
<script>
  export default {
    ...
    methods:{
      ...
      //削除処理
      delEstData(index , row){
        var p_est_hid = row.est_hid;
        var p_est_no = row.est_no;
        	...
        	var _this = this;
        	   _this.$http.get('/api/est/deliveryEstDoc',
        	      ...
        	      .then(function (response) {
                    var errorcode=response.data.errorcode;
                      if(errorcode=="200"){
                      // _this.$set(row,'delivery_dt',response.data.data);
                      _this.getList();
				          _this.$message({
				            type: 'success',
				            message: '送付が成功しました!'
      ...
      //詳細画面へ遷移
       goToDetail(index , row){
        this.$router.push({ path: 'EstDetail', query: { est_hid: row.est_hid }});
      },
   ...
   getList(){
 		      var _this = this;
          this.$http.post('/api/est/searchForDetail'
            ,{
